    ...
    public class WarriorAnimationEvents : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (_npcController == null)
                Debug.LogError(
                    $"[{gameObject.name}] WarriorAnimationEvents: NPCController component not found! Hitbox events will not work.",
                    this);
        ...
        public void Animation_EnableHitbox()
        {
            if (_npcController != null)
                ...
                Debug.LogError(
                    $"[{gameObject.name}] WarriorAnimationEvents: Attempted to call EnableHitbox, but NPCController is missing.",
                    this);
        }
        ...
        public void Animation_DisableHitbox()
        {
            if (_npcController != null)
                ...
                Debug.LogError(
                    $"[{gameObject.name}] WarriorAnimationEvents: Attempted to call DisableHitbox, but NPCController is missing.",
                    this);
        }
        ...
        public void Animation_StrikeComplete()
        {
            ...
            if (_npcController != null)
            {
                var arsenalItem = _npcController.GetCurrentArsenalItem();
                if (arsenalItem.HasValue)
                    EventManager.TriggerEvent(new AttackStrikeCompleteEventData
                    {
                        AttackerTransform = transform,
                        WeaponType = arsenalItem.Value.name
        ...
        public void Animation_RecoveryComplete()
        {
            ...
            if (_npcController != null)
            {
                var arsenalItem = _npcController.GetCurrentArsenalItem();
                if (arsenalItem.HasValue)
                    EventManager.TriggerEvent(new AttackRecoveryCompleteEventData()
                    {
                        AttackerTransform = transform,
                        WeaponType = arsenalItem.Value.name
